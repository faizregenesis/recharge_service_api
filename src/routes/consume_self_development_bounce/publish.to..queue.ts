...
const bounceCreateSelfDev  = `${process.env.BOUNCE_SELF_DEV_DATA_CREATE}`;
const bounceUpdateSelfDev  = `${process.env.BOUNCE_SELF_DEV_DATA_UPDATE}`;
const bounceDeleteSelfDev  = `${process.env.BOUNCE_SELF_DEV_DATA_DELETE}`;
...
const bounceDeleteSelfDevToAdmin = async (message: any) => {
    try {
        const connection = await amqp.connect(`${connectionUrl}`);
        const channel = await connection.createChannel();

        await channel.assertExchange(bounceDeleteSelfDev, 'fanout', { durable: true });

        const messageString = typeof message === 'string' ? message : JSON.stringify(message);
        channel.publish(bounceDeleteSelfDev, '', Buffer.from(messageString));
        console.log(`Delete self dev spread to pods and admin bounce: ${message}`);
